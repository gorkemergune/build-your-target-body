...
import type { BodyFatLog } from "@/types";
import { Percent, Trash2 } from "lucide-react";

type Period = "30" | "90" | "all";

export default function BodyFatPage({ params: { locale } }: { params: { locale: string } }) {
  ...
  const fetchLogs = () => api.get("/api/v1/body-fat").then((r) => setLogs(r.data));
  useEffect(() => { fetchLogs(); }, []);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      await api.post("/api/v1/body-fat", {
        body_fat_pct: parseFloat(pct),
  ...
  async function handleDelete(id: number) {
    ...
    setDeleting(id);
    try {
      await api.delete(`/api/v1/body-fat/${id}`);
      fetchLogs();
  ...
  const stats = useMemo(() => {
    ...
    const change = latest.body_fat_pct - first.body_fat_pct;
    ...
    return { latest: latest.body_fat_pct, change, weeklyAvg, total: logs.length };
  }, [logs]);
  ...
  const chartData = useMemo(() => {
    ...
    if (period === "all") {
      return sorted.map((l) => ({ date: l.logged_at.slice(0, 10), value: l.body_fat_pct }));
    }
    const cutoff = new Date();
    cutoff.setDate(cutoff.getDate() - (period === "30" ? 30 : 90));
    return sorted
      .filter((l) => new Date(l.logged_at) >= cutoff)
      .map((l) => ({ date: l.logged_at.slice(0, 10), value: l.body_fat_pct }));
  }, [logs, period]);

  const statItems = stats
    ? [
        { label: t("latest"), value: `${formatNumber(stats.latest)} %` },
        { label: t("totalChange"), value: formatChange(stats.change, " %"), highlight: true },
        { label: t("weeklyAvg"), value: formatChange(stats.weeklyAvg, " %") },
    ...
    <AppLayout locale={locale}>
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <Card>
            <CardHeader><CardTitle>{t("logBodyFat")}</CardTitle></CardHeader>
            <CardContent>
              <form onSubmit={handleSubmit} className="space-y-4">
                <div className="space-y-2">
                  <Label>{t("bodyFatPct")}</Label>
                  <Input
          ...
          <Card>
            ...
            <CardContent>
              {chartData.length > 0 ? (
                <TrendChart data={chartData} color="#f97316" unit="%" />
              ) : (
              ...
              <div className="divide-y">
                {logs.map((l) => (
                  <div key={l.id} className="py-3 flex justify-between items-center">
                    <div>
                      <p className="font-semibold">{l.body_fat_pct}%</p>
